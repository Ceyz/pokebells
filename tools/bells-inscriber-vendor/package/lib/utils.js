...
export function gptFeeCalculate(inputCount, outputCount, feeRate) {
    ...
    const transactionWeight = BASE_TX_WEIGHT + inputCount * INPUT_WEIGHT + outputCount * OUTPUT_WEIGHT;
...
export const getWintessUtxo = (utxo, addressType, publicKey, network) => {
    const value = Transaction.fromBuffer(Buffer.from(utxo.hex, "hex")).outs[utxo.vout].value;
    switch (addressType) {
